			...
			for (AdminVO adminVO : adminVOList) {
				if (account.equals(adminVO.getAdminAccount()) && password.equals(adminVO.getAdminPassword())) {
					if (adminVO.getAdminAccountStatus().intValue() == 0) {
						...
						res.sendRedirect(req.getContextPath()+"/back_end/admin/adminManagement.jsp");  //*工作3: (-->如無來源網頁:則重導至login_success.jsp)
						return;
					}
				}
			}
...
//    			session.invalidate();
		...
		if ("add".equals(action)) {
			...
			String accountReg = "^[(a-zA-Z0-9_)]{1,20}$";
			System.out.println(account);
			if (account == null || account.trim().length() == 0) {
				errorMsgs.add("帳號請勿空白");
			} else if (!account.trim().matches(accountReg)) {
				errorMsgs.add("帳號只能是英文字母、數字和_ , 且長度必需在1到20之間");
			}
