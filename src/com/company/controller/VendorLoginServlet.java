...
//					errorMsgs.add(e.getMessage());
		...
		if ("register".equals(action)) {
			...
			try {
				 ...
				 * 3.開始新增資料,寄送開通郵件
				 ***************************************/
				companyVO = companySvc.addCompany(companyStatus, headName, companyName, account, password, email, phone,
						bankAccount, address, companyRegisterTime);
				...
				e.printStackTrace();
//					errorMsgs.add(e.getMessage());
				RequestDispatcher failureView = req.getRequestDispatcher("/back_end/companyProduct/jsp/register.jsp");
				failureView.forward(req, res);
			}
